...
#----------------------------------------------------
TARGET = firmware
SRCS = main.c syscalls.c startup.c
...
#----------------------------------------------------
# === NEW: Disassemble the final .elf file into a .asm file ===
# This is the most useful one, as it interleaves C code with assembly
disasm: $(TARGET).elf
	$(OBJDUMP) -d -S $< > $(TARGET).asm

# === NEW: Generate .s assembly files for all C sources ===
assembly: $(ASSEMBLY)
